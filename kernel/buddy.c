...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].alloc, (NBLK(k) - 1) / 2 + 1);
    if (k > 0) {
      printf("  split:");
...
int bd_initfree_pair(int k, int bi, short left) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    const int flags_cnt = (NBLK(k) - 1) / 2 + 1;
    sz = sizeof(char) * ROUNDUP(flags_cnt, 8) / 8;
    bd_sizes[k].alloc = p;
